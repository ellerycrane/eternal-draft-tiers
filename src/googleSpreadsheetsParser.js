let GoogleSpreadsheetsParser, GoogleSpreadsheetsUtil
...
GoogleSpreadsheetsUtil = (function() {
  ...
  GoogleSpreadsheetsUtil.prototype.makeColumns = function(feedEntry) {
    var cell, columnCount, columns, j, len, obj, row, rowNumber, colIdx;
    columns = [];
    if (!(feedEntry.length >= 1 && feedEntry[0].gs$cell)) {
      return columns;
    }
    ...
    for (j = 0, len = feedEntry.length; j < len; j++) {
      obj = feedEntry[j];
      cell = obj.gs$cell;
      colIdx = Number(cell.col)
      if(!columns[colIdx]){
        columns[colIdx] = []
      }
      columns[colIdx].push(cell.$t)
      // if (Number(cell.row) !== 1) {
      //   if (cell.row !== rowNumber) {
      //     rowNumber = cell.row;
      //     row = [];
...
GoogleSpreadsheetsParser = (function() {
  function GoogleSpreadsheetsParser(publishedUrl, option) {
    ...
    this.columns = _util.makeColumns(feedEntry);
  }
...
export default GoogleSpreadsheetsParser